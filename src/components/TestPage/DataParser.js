...
export const parseData = (data) => {
  if (!data || typeof data !== 'object') return { summaryData: {}, voiceData: {} };
  ...
  Object.keys(data).forEach((slideKey, index) => {
    ...
    const pageNumber = pageNumberFromKey ? parseInt(pageNumberFromKey[0]) : (index + 1);
    ...
    if (slideData.Segments && typeof slideData.Segments === 'object') {
      ...
      const segments = Object.keys(slideData.Segments).map((segmentKey) => {
        ...
        return {
          ...
          isImportant: Boolean(segment.isImportant),
