...
export const summaryData = {
  ...
  2: `# 🔐 대칭 암호화 (Symmetric Encryption) 개념과 종류

## 📌 대칭 암호화 기본 개념
- **핵심 가정**: 송신자와 수신자가 **동일한 키**를 공유함
...
- **특징**:
  - 키 재사용 가능
  - 더 일반적으로 사용됨
  - 전자 코드북 모드(ECB) 등 다양한 운영 모드 존재
- **주요 예시**: DES, AES, ARIA, SEED, 치환 암호, 전치 암호

### 블록 암호 설계 특성
- 대칭 블록 암호는 **키에 의해 제어되는 치환(substitution)과 순열(permutation)로 구성된 라운드 시퀀스**로 구성
- **주요 설계 매개변수**:
  - 📏 블록 크기
  - 🔑 키 크기
  - 🔄 라운드 수
...
- **작동 방식**: 
  - 키(K)로 생성된 의사 난수 바이트 스트림(k)을 평문(M)과 XOR 연산
- **주요 예시**: LFSR 암호, RC4, ChaCha
